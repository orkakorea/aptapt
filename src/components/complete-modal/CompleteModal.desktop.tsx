...
function SeatInquiryTable({ data }: { data: ReceiptSeat }) {
  ...
  const rows = Array.from({ length }).map((_, i) => {
    const p = detailsItems[i] ?? {};
    const s = snapshotItems[i] ?? {};

    // 표기용 이름
    const aptName =
      getVal(p, ["apt_name", "aptName", "name", "apt", "title"]) ??
      getVal(s, ["apt_name", "aptName", "name", "apt", "title"]) ??
      topFallback ??
      "-";

    const productName =
      getVal(p, ["productName", "product_name", "mediaName"]) ??
      getVal(s, ["productName", "product_name", "mediaName"]) ??
      "-";

    // 기간(개월)
    const months =
      parseNumberLoose(getVal(p, ["months", "month"])) ?? parseNumberLoose(getVal(s, ["months", "month"])) ?? 0;
    const periodLabel = months ? `${months}개월` : getVal(p, ["period", "periodLabel"], "-");
    ...
    const baseMonthly =
      parseNumberLoose(
        getVal(
          p,
          ["baseMonthly", "base_monthly", "priceMonthly"],
          getVal(s, ["baseMonthly", "base_monthly", "priceMonthly"]),
        ),
      ) ?? 0;
    ...
    const monthlyAfter =
      ...
      // 라인합계가 있으면 역산
      (parseNumberLoose(getVal(p, ["lineTotal", "item_total_won", "total_won"])) && months
        ? Math.round((parseNumberLoose(getVal(p, ["lineTotal", "item_total_won", "total_won"])) as number) / months)
        : parseNumberLoose(getVal(s, ["lineTotal", "item_total_won", "total_won"])) && months
          ? Math.round((parseNumberLoose(getVal(s, ["lineTotal", "item_total_won", "total_won"])) as number) / months)
          : undefined);

    // 기준금액(정가×개월)
    const baseTotal = baseMonthly && months ? Math.round(baseMonthly * months) : 0;

    // 총광고료(우선순위: 라인 합계 → (할인후월가×개월) → (정가×개월))
    let lineTotal =
      parseNumberLoose(getVal(p, ["lineTotal", "line_total", "item_total_won", "total_won"])) ??
      parseNumberLoose(getVal(s, ["lineTotal", "line_total", "item_total_won", "total_won"]));

    if (!isFinite(Number(lineTotal)) || Number(lineTotal) <= 0) {
      if (typeof monthlyAfter === "number" && months) lineTotal = Math.round(monthlyAfter * months);
      else if (baseTotal) lineTotal = baseTotal;
      else lineTotal = 0;
    }

    // 할인율 = 1 - (총광고료 / 기준금액)
    let discountPct: string | "-" = "-";
    if (baseTotal > 0 && isFinite(Number(lineTotal))) {
      const rate = 1 - Number(lineTotal) / baseTotal;
      const clipped = Math.max(0, Math.min(0.9, rate)); // 이상치 가드
      discountPct = `${Math.round(clipped * 100)}%`;
    }

    // 화면 표기용 “월광고료” 컬럼: 할인후월가 → (라인합계/개월) → 정가
    const monthlyFee =
      (typeof monthlyAfter === "number" ? monthlyAfter : undefined) ??
      (months ? Math.round((lineTotal as number) / months) : undefined) ??
      (baseMonthly || 0);
  ...
  const totals = rows.reduce(
    (acc, r) => {
      const n = (obj: any, keys: string[]) => {
        const v = parseNumberLoose(getVal(obj, keys));
        return v && isFinite(v) ? v : 0;
      };
      acc.households += n(r._rawP, ["households", "household", "hh"]) || n(r._rawS, ["households", "household", "hh"]);
      acc.residents += n(r._rawP, ["residents", "population"]) || n(r._rawS, ["residents", "population"]);
      acc.monthlyImpressions +=
        n(r._rawP, ["monthlyImpressions", "monthly_impressions", "impressions", "plays"]) ||
        n(r._rawS, ["monthlyImpressions", "monthly_impressions", "impressions", "plays"]);
      acc.monitors +=
        n(r._rawP, ["monitors", "monitor_count", "monitorCount", "screens"]) ||
        n(r._rawS, ["monitors", "monitor_count", "monitorCount", "screens"]);
      return acc;
    },
  ...
  const vat = Math.round(periodTotal * 0.1);
  const grand = periodTotal + vat;
      ...
      <div className="px-4 py-4">
        <div className="rounded-xl border border-[#E5E7EB] bg-[#F7F5FF]">
          ...
          <div className="flex items-center justify-between border-t border-[#E5E7EB] px-4 py-3">
            ...
            <span className="text-sm font-bold text-red-500">{formatKRW(vat)}</span>
          </div>
          <div className="flex items-center justify-between border-t border-[#E5E7EB] px-4 py-4">
            ...
            <span className="text-[20px] font-extrabold text-[#6C2DFF]">{formatKRW(grand)}</span>
          </div>
        </div>
      </div>
    </div>
