    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          <motion.div
            key="sheet"
            className="fixed inset-x-0 bottom-0 z-[100] max-h-[88vh] overflow-hidden rounded-t-2xl bg-white shadow-[0_-10px_30px_rgba(0,0,0,0.12)]"
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
            ...
            <AnimatePresence>
              {pickerOpen && (
                <>
                  ...
                  <motion.div
                    key="picker"
                    className="fixed inset-x-0 bottom-0 z-[120] rounded-t-2xl bg-white shadow-[0_-8px_24px_rgba(0,0,0,0.12)]"
                    initial={{ y: "100%" }}
                    animate={{ y: 0 }}
                    exit={{ y: "100%" }}
                    transition={{ type: "spring", stiffness: 360, damping: 30 }}
