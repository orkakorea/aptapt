...
export default function QuoteMiniTable({
  ...
  const vat = useMemo(
    () => (showVatBreakdown ? Math.round(subtotal * (Number(vatRate) || 0)) : 0),
    [showVatBreakdown, subtotal, vatRate],
  );
          ...
          {showFooterTotal && (
            <tfoot>
              <tr className="border-t border-[#E5E7EB]">
                ...
                <td className="px-4 py-4 bg-[#F7F5FF] text-right font-bold text-[#6C2DFF]">{fmtWon(subtotal)}</td>
              </tr>

              {showVatBreakdown && (
                <>
                  <tr className="border-t border-[#E5E7EB]">
                    <td colSpan={6} className="text-right px-4 py-3 bg-[#F7F5FF] text-[#6B7280]">
                      부가세
                    </td>
                    <td className="px-4 py-3 bg-[#F7F5FF] text-right text-[#111827]">{fmtWon(vat)}</td>
                  </tr>
                  <tr className="border-t border-[#E5E7EB]">
                    <td colSpan={6} className="text-right px-4 py-4 bg-[#F7F5FF] text-[#6B7280] font-medium">
                      최종광고료 <span className="text-[#9CA3AF]">(VAT 포함)</span>
                    </td>
                    <td className="px-4 py-4 bg-[#F7F5FF] text-right font-extrabold text-[#6C2DFF]">
                      {fmtWon(grandTotal)}
                    </td>
                  </tr>
                </>
