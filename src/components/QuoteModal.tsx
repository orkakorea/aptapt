  ...
  const inquiryPrefill = useMemo(() => {
    ...
    const cart_snapshot = {
      ...
      items: (items ?? []).map((it) => {
        ...
        const baseMonthly = toNum(it.baseMonthly);
        const monthlyAfter = Math.round(baseMonthly * (1 - precompRate) * (1 - periodRate));
        const lineTotal = monthlyAfter * toNum(it.months);

        // 카운터용 수치(없으면 0)
        const households = toNum(it.households);
        const residents = toNum(it.residents);
        const monthlyImpressions = toNum(it.monthlyImpressions);
        const monitors = toNum(it.monitors);
