...
const InquiriesPage: React.FC = () => {
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      // campaign filter
      if (campaignFilter) {
        q = q.eq("campaign_type", campaignFilter);
      }
      // date filters
      if (dateFrom) {
  ...
  const uniqueCampaigns = useMemo(() => {
    ...
    rows.forEach((r) => {
      if (r.campaign_type) set.add(r.campaign_type);
    });
  ...
  const updateSelected = async (patch: Partial<Inquiry>) => {
    ...
    try {
      ...
      setSelected(next);
      // refresh logs (trigger may have added rows)
...
const lines = rows.map((r) =>
  cols
    .map((k) => {
      const v = (r as any)[k];
      const s = v == null ? "" : String(v).replace(/"/g, '""'); // 이스케이프
      return `"${s}"`; // 정상적으로 큰따옴표 감싸기
    })
    .join(",")
);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <button
            className="h-9 px-3 rounded-md bg-[#6C2DFF] text-white text-sm hover:bg-[#5a1fff]"
            onClick={exportCSV}
          >
            CSV 다운로드
          </button>
                ...
                return (
                  <tr key={r.id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-right">
                      <button
                        className="h-8 px-3 rounded-md border text-xs hover:bg-gray-50"
      ...
      {rows.length > PAGE_SIZE && (
        <div className="flex items-center justify-between pt-2">
          <div className="text-sm text-gray-500">
            총 {rows.length}건 / 페이지 {page} / {Math.ceil(rows.length / PAGE_SIZE)}
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              className="h-8 px-3 rounded-md border text-xs disabled:opacity-50"
              onClick={() => setPage((p) => (p * PAGE_SIZE >= rows.length ? p : p + 1))}
        ...
        <div className="fixed inset-0 z-[100]">
          ...
          <div className="absolute right-0 top-0 h-full w-full max-w-[560px] bg-white shadow-2xl p-5 overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button className="text-gray-500 hover:text-gray-700" onClick={closeDetail}>
                닫기
              </button>
            </div>
            ...
            <section className="space-y-3 mb-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  value={selected.admin_memo || ""}
                  onChange={(e) => updateSelected({ admin_memo: e.target.value })}
                  disabled={saving}
                />
              </div>

              <div className="text-xs text-gray-500 mt-1">
                완료 처리 시점: {formatDate(selected.processed_at)}
              </div>
            </section>
            ...
            <section className="mb-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                <pre className="bg-gray-50 rounded-lg p-3 overflow-auto text-[11px] leading-[1.3]">
                  {JSON.stringify(selected.cart_snapshot, null, 2)}
                </pre>
                <pre className="bg-gray-50 rounded-lg p-3 overflow-auto text-[11px] leading-[1.3]">
                  {JSON.stringify(selected.extra || selected.utm, null, 2)}
                </pre>
              </div>
            </section>
              ...
              <div className="space-y-2">
                ...
                {logs.map((l) => (
                  <div key={l.id} className="text-sm">
                    ...
                    <div>
                      <span className="inline-block min-w-[96px] text-xs mr-2 px-2 py-0.5 rounded-full border bg-gray-50">
                        {l.action}
                      </span>
